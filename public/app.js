 ...
 * DNA: Cerebro v12.5 Master Live (Sincronización Total + Doble Rol + Blindaje)
 */

const URL_SERVIDOR = window.location.origin;
const TOKEN_ACCESO = "carbonazo2024pro";

// --- 1. ESTADO GLOBAL (INICIALIZADO PARA EVITAR ERRORES) ---
...
// --- 2. ARRANQUE E INICIALIZACIÓN LIVE ---

window.onload = async () => {
    console.log("🔥 Motor Visual v12.5 Iniciado");
    
    // Conectar Socket.io de forma segura
    if (typeof io !== 'undefined') {
        socket = io();
        socket.on('actualizar_pantallas', async () => {
            // Solo refrescamos si no hay una cuenta abierta editándose (para no borrar el carrito actual)
            if (!subCuentaActiva) {
...
// --- 3. GESTIÓN DE ROLES Y PRIVACIDAD ---
...
async function refrescarMesas() {
    try {
        ...
        // Alerta de cocina lista (Si un estado cambió de Pendiente a Listo)
        nuevas.forEach(m => {
...
function renderizarAdminProductos() {
    ...
    if(cuerpo) cuerpo.innerHTML = productos.map(p => `<tr><td>${p.icono}</td><td>${p.nombre}</td><td>C$ ${parseFloat(p.precio).toFixed(2)}</td><td>${p.stock}</td><td><button onclick="prepararEdicion(${p.id})" style="color:blue; background:none; border:none; cursor:pointer;"><i class="fas fa-edit"></i></button><button onclick="borrarProducto(${p.id})" style="color:red; background:none; border:none;"><i class="fas fa-trash"></i></button></td></tr>`).join('');
}
...
async function guardarNuevoProducto() {
    ...
    const btn = document.querySelector("button[onclick='guardarNuevoProducto()']");
    if(btn) { btn.innerText = "OK"; btn.style.background = ""; }
...
async function procesarPegadoMasivo() {
    ...
    if(socket) socket.emit('notificar_cambio');
    alert("✅ Importación completada"); cerrarModal();
}

// --- 10. UTILIDADES Y CARGAS ---
...
async function cargarUsuariosLista() {
    try {
        ...
        const sM = document.getElementById('select-mesero');
        const lU = document.getElementById('login-usuario');
...
function validarSumaCombinada() {
    ...
    const av = document.getElementById('combinado-aviso');
    if (av) av.innerText = Math.abs(t-s) < 0.1 ? "✅ OK" : `Faltan: C$ ${(t-s).toFixed(2)}`;
