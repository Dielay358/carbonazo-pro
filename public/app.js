 ...
 * DNA: Cerebro v10.0 - LIVE REAL-TIME (Sincronización Total)
 */

const URL_SERVIDOR = window.location.origin;
const TOKEN_ACCESO = "carbonazo2024pro";

// --- CONEXIÓN LIVE (SOCKET.IO) ---
const socket = io(); // Se conecta al servidor al cargar

// --- ESTADO GLOBAL ---
let usuarioLogueado = "Mesero"; // Inicializado para evitar errores
...
// --- 1. INICIALIZACIÓN Y RECEPCIÓN LIVE ---
window.onload = async () => {
...
// ⚡ ESCUCHAR CAMBIOS EN VIVO DESDE OTROS DISPOSITIVOS
socket.on('actualizar_pantallas', async () => {
    // Solo refrescamos si el mesero no está en medio de una edición de cuenta
    if (!subCuentaActiva) {
        console.log("🔄 Sincronizando datos con otros dispositivos...");
        await refrescarMesas();
        await obtenerProductosDB();
    }
});
...
// --- 6. ACCIONES LIVE (ENVIAR NOTIFICACIÓN A OTROS) ---
...
// Generación de Ticket con Logo y Notas
...
// --- 8. ADMINISTRACIÓN (PRODUCTOS, MESEROS, ESTADÍSTICAS) ---
...
function renderizarAdminProductos() {
    document.getElementById('cuerpo-tabla-admin').innerHTML = productos.map(p => `<tr><td>${p.icono || '🍽️'}</td><td>${p.nombre}</td><td>C$ ${p.precio}</td><td>${p.stock}</td><td><button onclick="borrarProducto(${p.id})" style="color:red; background:none; border:none;"><i class="fas fa-trash"></i></button></td></tr>`).join('');
}
...
async function procesarPegadoMasivo() {
    ...
    for (let f of filas) {
        const cols = f.split('\t');
        if (cols.length >= 3) {
            lista.push({ categoria: cols[0], nombre: cols[1], precio: parseFloat(cols[2].replace(/[^0-9.]/g, '')), icono: '🍽️', stock: 999 });
        }
    }
...
async function borrarUsuario(id) { 
    if(confirm("¿Borrar mesero?")) {
        await fetch(`${URL_SERVIDOR}/usuarios-admin/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${TOKEN_ACCESO}` } });
        renderizarAdminUsuarios();
    }
}
...
async function confirmarBorrarVenta(id) { 
    if (confirm("¿Borrar venta?")) { 
        await fetch(`${URL_SERVIDOR}/borrar-venta/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${TOKEN_ACCESO}` } }); 
        socket.emit('cambio_en_sistema'); 
        abrirModalVentas(); 
    } 
}
