 ...
 * DNA: Cerebro v9.0 (Versión Final Sincronizada y Blindada)
 */
...
function generarFiltrosCategorias() {
    ...
    barra.innerHTML = cats.sort().map(c => `<button class="btn-filtro" onclick="filtrarPorCategoria('${c}')">${c}</button>`).join('');
}
...
function abrirPegarMasivo() {
    ...
    if (modal) { modal.style.display = 'flex'; document.getElementById('texto-pegado').value = ''; }
}
...
async function procesarPegadoMasivo() {
    ...
    alert("✅ Importación completada"); cerrarModal(); obtenerProductosDB();
}
...
function anularCuentaActual() { if (confirm("¿Anular mesa?")) { fetch(`${URL_SERVIDOR}/limpiar-mesa/${subCuentaActiva}`, { method: 'DELETE' }).then(() => { limpiarPantallaPostAccion(); refrescarMesas(); }); } }
...
function imprimirPreCuenta() {
    ...
    area.innerHTML = `<div class="ticket-header"><h3>PRE-CUENTA</h3><p>${new Date().toLocaleString()}</p></div><div class="ticket-divisor"></div>${items}<div class="ticket-divisor"></div><div class="ticket-fila"><span>Subtotal:</span><span>C$ ${t.toFixed(2)}</span></div><div class="ticket-total">Total + Propina: C$ ${(t*1.1).toFixed(2)}</div>`;
    setTimeout(() => window.print(), 300);
}
...
async function guardarNuevoProducto() { const d = { nombre: document.getElementById('nuevo-nombre').value, precio: parseFloat(document.getElementById('nuevo-precio').value), icono: document.getElementById('nuevo-icono').value, categoria: document.getElementById('nuevo-categoria').value, stock: parseInt(document.getElementById('nuevo-stock').value) }; await fetch(`${URL_SERVIDOR}/agregar-producto`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${TOKEN_ACCESO}` }, body: JSON.stringify(d) }); await obtenerProductosDB(); renderizarAdminProductos(); }
